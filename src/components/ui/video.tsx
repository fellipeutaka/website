...
export function VideoRoot({ children, className, ...props }: VideoRootProps) {
  ...
  function handleSkip(value: number) {
    if (videoRef.current) {
      setState((prevState) => ({
        ...prevState,
        currentTime: prevState.currentTime + value,
      }));
      videoRef.current.currentTime += value;
    }
  }
  ...
  useHotkeys([
    ["ArrowLeft", () => handleBack(5)],
    ["ArrowRight", () => handleSkip(5)],
    [
      "ArrowUp",
      () => handleVolumeChange(Math.min((playerState.volume * 10 + 1) / 10, 1)),
    ],
    [
      "ArrowDown",
      () => handleVolumeChange(Math.max((playerState.volume * 10 - 1) / 10, 0)),
    ],
    ["Space", togglePlaying],
    ["j", () => handleBack(10)],
    ["k", togglePlaying],
